...
export const TASK_HISTORY_CALENDAR_WEEKDAY_LABELS = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"] as const;

export type TaskHistoryCalendarHistoryEntry = {
  detail?: ReactNode;
  key: string;
  label: ReactNode;
  status: ReactNode;
...
export type TaskHistoryCalendarPresentationProps = {
  ariaLabel: string;
  description: ReactNode;
  historyEntries: readonly TaskHistoryCalendarHistoryEntry[];
...
export function getTaskHistoryCalendarMonthDays(monthKey: string) {
...
export function shiftTaskHistoryCalendarMonth(monthKey: string, amount: number) {
  const [year, month] = monthKey.split("-").map(Number);
  const date = new Date(Date.UTC(year, month - 1 + amount, 1));
  return `${date.getUTCFullYear()}-${String(date.getUTCMonth() + 1).padStart(2, "0")}`;
}

export function formatTaskHistoryCalendarDay(logicalDay: string, timezone: string) {
  return new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeZone: timezone }).format(new Date(`${logicalDay}T12:00:00Z`));
}

export function formatTaskHistoryCalendarMonth(monthKey: string, timezone: string) {
  return new Intl.DateTimeFormat(undefined, { month: "long", year: "numeric", timeZone: timezone }).format(new Date(`${monthKey}-01T12:00:00Z`));
}

export function TaskHistoryCalendarDay({
  ariaLabel,
  day,
  disabled = false,
  muted = false,
...
export function TaskHistorySummaryStat({ label, value }: { label: string; value: string }) {
...
export function TaskHistorySelectedDayPresentation({
  action,
  children,
  label,
  status,
    ...
    <div className="mt-4 border-t border-[#eee9f8] pt-3 dark:border-white/10">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="grid grid-cols-4 gap-1.5 text-right">
          {summaryStats.map((stat) => <TaskHistorySummaryStat key={stat.label} {...stat} />)}
        </div>
      </div>
...
export function TaskHistoryCalendarPresentation({
  ariaLabel,
  description,
  historyEntries,
  historyDescription,
  ...
  selectedDayLabel,
  selectedDayStatus,
}: TaskHistoryCalendarPresentationProps) {
  return (
