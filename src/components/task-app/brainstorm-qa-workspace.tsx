...
type Confirmation = "clear-passed" | "delete-session" | "replace" | "reset" | null;
...
export function BrainstormQaWorkspace({ appVersion, qaState, updateQaState }: {
  ...
  return (
    <div className="space-y-4 pb-[max(2rem,env(safe-area-inset-bottom))]">
      <AdhdPanel padding="lg">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <TaskTableChipButton disabled={!progress?.pass} onClick={() => requestConfirmation("clear-passed")}>Clear Passed</TaskTableChipButton>
      ...
      {confirmation ? <ModalShell label={confirmation === "reset" ? "Reset QA session" : confirmation === "delete-session" ? "Delete QA session" : confirmation === "clear-passed" ? "Clear passed QA items" : "Replace checklist items"} onClose={closeConfirmation}><AdhdPanel className="w-[min(92vw,30rem)]" padding="lg"><h3 className="text-lg font-semibold text-[#342d53] dark:text-white">{confirmation === "reset" ? "Reset for retesting?" : confirmation === "delete-session" ? "Delete this QA session?" : confirmation === "clear-passed" ? "Clear passed items?" : "Replace checklist items?"}</h3><p className="mt-2 text-sm leading-6 text-[#716b8c] dark:text-white/60">{confirmation === "reset" ? "Every item will return to Not Tested. Item notes, metadata, order, and general observations will remain." : confirmation === "delete-session" ? "This permanently removes this QA session and its checklist, notes, and observations. Questionnaire content and other QA sessions will remain." : confirmation === "clear-passed" ? `${progress?.pass ?? 0} passed item${progress?.pass === 1 ? "" : "s"} will be permanently removed. Fail, Blocked, and Not Tested items will remain.` : "The current item list will be replaced. Session metadata and general observations will remain."}</p><div className="mt-5 flex justify-end gap-2"><TaskTableChipButton onClick={closeConfirmation}>Cancel</TaskTableChipButton><TaskTableChipButton onClick={() => { if (confirmation === "reset") { setActiveSession(resetQaSession(activeSession)); setFeedback("Session reset for retesting. Notes and observations were preserved."); } else if (confirmation === "delete-session") { updateQaState(deleteQaSession(qaState, activeSession.id)); setExpandedItemIds(new Set()); setImportVersionSessionId(null); setFeedback("QA session deleted."); } else if (confirmation === "clear-passed") { const passCount = progress?.pass ?? 0; setActiveSession(clearPassedQaItems(activeSession)); setExpandedItemIds((current) => new Set([...current].filter((itemId) => activeSession.items.some((item) => item.id === itemId && item.status !== "pass")))); setFeedback(`${passCount} passed item${passCount === 1 ? "" : "s"} cleared.`); } else runImport("replace"); closeConfirmation(); }} toneClassName={confirmation === "reset" ? "border-amber-200 bg-amber-50 text-amber-800" : "border-rose-200 bg-rose-50 text-rose-700"}>Confirm</TaskTableChipButton></div></AdhdPanel></ModalShell> : null}
    </div>
  );
}
