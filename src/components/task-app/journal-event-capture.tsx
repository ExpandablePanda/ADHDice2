  ...
  return createPortal(
    ...
    >
      <div className="grid gap-2">
        ...
        <div className="grid gap-1.5"><p className={`${QUESTION_HINT_CLASS} text-center font-semibold uppercase tracking-[0.16em]`}>{signal.kind === "symptom" ? "Severity" : "Intensity"} · 1–{denominator}</p><div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">{Array.from({ length: denominator }, (_, index) => index + 1).map((score) => <button aria-label={`${displayName} ${score}, ${signal.scale_labels[score] ?? ""}`} aria-pressed={overlay.score === score} className={`flex min-h-8 w-full min-w-0 items-start justify-start gap-2 rounded-[0.7rem] border px-2 py-1.5 text-left text-xs font-semibold ${overlay.score === score ? "border-[#5d49c7] bg-[#6f57f6] text-white dark:border-[#cabfff] dark:bg-[#cabfff] dark:text-[#1a1431]" : "border-[#6f57f6] bg-white text-[#615b9c] dark:bg-white/8 dark:text-white/65"}`} key={score} onClick={() => onChange({ error: null, score })} type="button"><span className="shrink-0 font-semibold">{score}</span><span className="min-w-0 flex-1 break-words whitespace-normal">{signal.scale_labels[score] ?? ""}</span></button>)}</div></div>
