      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-2xl">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-3">
            ...
            <button className="ui-pill-button-light transition hover:bg-[#f6f1ff] dark:border-white/15 dark:text-[#cabfff] dark:hover:bg-white/8" onClick={onToggleCollapsed} type="button">
        ...
        <div className="flex flex-wrap gap-2">
          <button className="ui-pill-button-strong-light dark:bg-[#261e49] dark:text-[#cabfff]" onClick={onOpenFocusPlanner} type="button">
            Open Focus Planner
          </button>
          {missedCount > 0 ? (
            <button className="ui-pill-button-danger-light dark:bg-[#44232f] dark:text-[#ff9eaf]" onClick={() => onSelectBucket("missed")} type="button">
              Review Missed
            </button>
          ) : null}
        </div>
        ...
        <div className="mt-4 grid gap-2">
          {planningCandidates.map((task) => (
            <div className="flex flex-col gap-3 rounded-[1rem] border border-[#f0ebfb] bg-[#fcfbff] px-4 py-3 sm:flex-row sm:items-center sm:justify-between dark:border-white/10 dark:bg-white/[0.04]" key={task.id}>
              ...
              <div className="flex flex-wrap gap-2">
                <button className="ui-pill-button-strong-light dark:bg-[#261e49] dark:text-[#cabfff]" onClick={() => routeTaskToToday(task.id)} type="button">
                  Plan Today
                </button>
                <button className="ui-pill-button-light dark:bg-white/8 dark:text-white/65" onClick={() => sendTaskToWaiting(task.id)} type="button">
                  Waiting
                </button>
                <button className="ui-pill-button-light dark:bg-white/8 dark:text-white/65" onClick={() => sendTaskToLater(task.id)} type="button">
                  Later
                </button>
                <button className="ui-pill-button-light border-[#f2df9b] bg-[#fff6df] text-[#a87200] dark:bg-[#44350d] dark:text-[#ffd36c]" onClick={() => onSetTaskRecurring(task.id, "weekly")} type="button">
                  Repeat Weekly
                </button>
              </div>
            </div>
