...
import type { HealthExercise } from "@/lib/database.types";
import {
...
export function HealthFitnessSessionEditor({ draft, exerciseLibrary, onChange }: HealthFitnessSessionEditorProps) {
  ...
  function addExercise() {
    ...
    if (!exercise) return;
    onChange({ exercises: [...draft.exercises, createHealthWorkoutExerciseDraft(exercise)] });
  }
...
function WorkoutExerciseDraftRow({
  ...
  const currentLibraryExercise = exerciseLibrary.find((candidate) => candidate.id === exercise.exerciseId);
  const exerciseOptions = [
    ...(currentLibraryExercise && currentLibraryExercise.archived_at !== null ? [{ label: `${currentLibraryExercise.name} (archived)`, value: currentLibraryExercise.id }] : []),
    ...exerciseLibrary.filter((candidate) => candidate.archived_at === null && candidate.id !== currentLibraryExercise?.id).map((candidate) => ({ label: candidate.name, value: candidate.id })),
  ];
  ...
  function changeExercise(exerciseId: string) {
    ...
    onChange({
      ...
      measurementType: nextExercise.default_measurement,
    ...
    <article className="grid gap-3 rounded-[1rem] border border-[#e3ddf6] bg-white/80 p-3 dark:border-white/10 dark:bg-white/[0.04]">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2">
          ...
          <AdhdChip className="pointer-events-none" tone="purple" type="button">{exercise.measurementType === "reps" ? "Reps" : "Duration"}</AdhdChip>
        </div>
