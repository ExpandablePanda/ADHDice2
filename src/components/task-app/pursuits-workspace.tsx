...
import { AdhdCard, AdhdChip, AdhdDropdownSelect, AdhdIconButton, AdhdPanel, EditableEntityHeaderTitle } from "@/components/ui-system";
  ...
  return (
    <AdhdPanel className="min-w-0" title="Pursuits" subtitle="Ongoing interests and skills, with completion-based revisit targets.">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <AdhdChip onClick={() => setIsManagerOpen((current) => !current)} selected={isManagerOpen} type="button">{isManagerOpen ? "Hide manager" : "Manage pursuits"}</AdhdChip>
      ...
      {isManagerOpen ? (
        <div className="mt-5 border-t border-[#eee9f8] pt-4 dark:border-white/10">
          <div className="flex items-center justify-between gap-2"><p className="text-xs font-semibold text-[#655d7d] dark:text-white/72">All Pursuits</p><AdhdChip onClick={() => { void onRefresh(); }} type="button">Refresh</AdhdChip></div>
  ...
  return (
    <ModalShell className="adhdice-scrollbar flex h-[100dvh] w-full max-w-6xl flex-col overflow-hidden rounded-none border border-[#ece8f8] bg-white p-4 shadow-[0_30px_80px_rgba(81,61,168,0.18)] sm:h-auto sm:max-h-[calc(100vh-2rem)] sm:rounded-[2.4rem] sm:p-6 dark:border-white/10 dark:bg-[#171328]" label={pursuit ? `Edit Pursuit ${pursuit.title}` : "New Pursuit"} onClose={isPending ? undefined : onClose}>
      <form className="flex min-h-0 flex-1 flex-col" onSubmit={handleSubmit}>
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-[#eee9f8] pb-4 dark:border-white/10"><div className="flex min-w-0 items-start gap-3"><div className="mt-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-[0.9rem] bg-[#f0ebff] text-[#6f57f6] dark:bg-[#2b214c] dark:text-[#cabfff]"><Compass className="h-5 w-5" /></div><div className="min-w-0"><p className="text-[11px] font-medium uppercase tracking-[0.22em] text-[#9b92be] dark:text-white/35">Pursuit</p><EditableEntityHeaderTitle aria-label="Pursuit title" onChange={setTitle} placeholder="Name this Pursuit" value={title} /></div></div><div className="flex shrink-0 items-center gap-1"><AdhdChip tone={getStatusTone(status)}>{formatStatusLabel(status)}</AdhdChip><AdhdIconButton aria-label="Calendar" onClick={() => setActiveSection("calendar")} size="sm" title="Calendar" variant="rowToolbar"><CalendarDays /></AdhdIconButton><AdhdIconButton aria-label="Close" disabled={isPending} onClick={onClose} size="sm" title="Close" variant="rowToolbar"><X /></AdhdIconButton></div></header>
        ...
        <footer className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-t border-[#eee9f8] pt-4 dark:border-white/10"><div className="flex items-center gap-1"><AdhdChip onClick={() => setActiveSection("overview")} selected={activeSection === "overview"} type="button">Overview</AdhdChip><AdhdChip icon={<CalendarDays className="h-3.5 w-3.5" />} onClick={() => setActiveSection("calendar")} selected={activeSection === "calendar"} type="button">Calendar</AdhdChip></div><div className="flex justify-end gap-2"><AdhdChip disabled={isPending} onClick={onClose} type="button">Cancel</AdhdChip><button className={PRIMARY_BUTTON_CLASS} disabled={isPending} type="submit">{isPending ? "Saving..." : pursuit ? "Save Pursuit" : "Create Pursuit"}</button></div></footer>
      </form>
