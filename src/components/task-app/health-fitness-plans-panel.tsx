      ...
      <div className="grid gap-4">
        ...
        {editor ? (
          <div aria-label={editor.id ? "Edit Fitness Plan" : "Create Fitness Plan"} className="grid gap-4 rounded-[1.25rem] border border-[#ddd2ff] bg-[#fbfaff] p-4 dark:border-[#42306f] dark:bg-white/[0.04]" role="dialog">
            ...
            <div className="grid gap-2">
              <div><h4 className="text-xs font-bold uppercase tracking-[0.14em] text-[#6f57f6] dark:text-[#cabfff]">Planned Items</h4><p className="mt-1 text-xs text-[#7d7598] dark:text-white/50">Multiple items may use the same weekday.</p></div>
            ...
            <div className="flex flex-wrap gap-2"><AdhdChip disabled={isSaving} icon={<Plus aria-hidden="true" className="h-3.5 w-3.5" />} onClick={() => setEditor((current) => current ? { ...current, items: [...current.items, createDefaultPlanItem(createPlanTypes[0] ?? "Other")] } : current)} type="button">Add Planned Item</AdhdChip><AdhdChip disabled={isSaving} onClick={() => { void handleSavePlan(); }} tone="purple" type="button">{isSaving ? "Saving…" : "Save Plan"}</AdhdChip><AdhdChip disabled={isSaving} onClick={() => setEditor(null)} type="button">Cancel</AdhdChip></div>
          </div>
        ) : null}
      </div>
    </HealthCollapsiblePanel>
