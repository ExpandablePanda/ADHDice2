...
export function HealthFitnessExerciseLibrary({
  ...
  const [measurementDraft, setMeasurementDraft] = useState<HealthFitnessMeasurement>("reps");
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editingName, setEditingName] = useState("");
  const [editingMeasurement, setEditingMeasurement] = useState<HealthFitnessMeasurement>("reps");
  ...
  async function handleAddExercise(event: FormEvent<HTMLFormElement>) {
    ...
    setIsSaving(true);
    const created = await createExercise({ default_measurement: measurementDraft, name: nameDraft });
    setIsSaving(false);
    if (created) {
      setNameDraft("");
      setMeasurementDraft("reps");
    }
  }
  ...
  function beginEdit(exercise: HealthExercise) {
    ...
    setEditingMeasurement(exercise.default_measurement);
  }

  async function handleSaveEdit(exerciseId: string) {
    setIsSaving(true);
    const saved = await updateExercise(exerciseId, { default_measurement: editingMeasurement, name: editingName });
    setIsSaving(false);
    if (saved) {
      setEditingId(null);
      setEditingName("");
...
function ExerciseLibraryRow({
  ...
  editingMeasurement?: HealthFitnessMeasurement;
  editingName?: string;
  isSaving?: boolean;
  onArchive?: () => void;
  onCancel?: () => void;
  onEdit?: () => void;
  onSave?: () => void;
  setEditingMeasurement?: (value: HealthFitnessMeasurement) => void;
  setEditingName?: (value: string) => void;
}) {
  if (editing && !archived && editingMeasurement && editingName !== undefined && setEditingMeasurement && setEditingName) {
    return (
  ...
  return (
    <div className={`flex flex-wrap items-center justify-between gap-2 rounded-[0.9rem] border px-3 py-2 ${archived ? "border-[#eeeaf8] bg-[#faf9fd] dark:border-white/10 dark:bg-white/[0.02]" : "border-[#eeeaf8] bg-white/80 dark:border-white/10 dark:bg-white/[0.03]"}`}>
      <div className="min-w-0">
        ...
        <p className="mt-0.5 text-xs text-[#8d87a7] dark:text-white/40">{exercise.default_measurement === "reps" ? "Reps" : "Duration"}{archived ? " · Archived" : ""}</p>
      </div>
      {!archived ? (
