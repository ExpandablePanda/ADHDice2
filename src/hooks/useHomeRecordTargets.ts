...
import { normalizeRecordsLogicalDayStart } from "@/lib/records/freshness";

type HomeRecordTargetRow = Pick<PersistedRecordCurrent, "logical_day_start" | "metric_key" | "recalculated_at" | "timezone" | "value">;
...
const EMPTY_STATE: HomeRecordTargetsState = { error: null, loading: false, recalculatedAt: null, settingsMismatch: false, targets: {} };
...
function normalizeStoredDayStart(value: string) {
  return normalizeRecordsLogicalDayStart(value);
}
...
export function homeRecordSettingsMatch(row: Pick<HomeRecordTargetRow, "logical_day_start" | "timezone">, settings: { logicalDayStart: string; timezone: string }) {
  const rowLogicalDayStart = normalizeStoredDayStart(row.logical_day_start);
  const settingsLogicalDayStart = normalizeStoredDayStart(settings.logicalDayStart);
  return Boolean(rowLogicalDayStart && settingsLogicalDayStart)
    && row.timezone === settings.timezone
    && rowLogicalDayStart === settingsLogicalDayStart;
}
...
export async function loadHomeRecordTargets(
...
): Promise<Pick<HomeRecordTargetsState, "recalculatedAt" | "settingsMismatch" | "targets">> {
  const result = await client
    .from("adhdice_record_current")
    .select("metric_key,value,timezone,logical_day_start,recalculated_at")
    .eq("user_id", userId)
  ...
  for (const row of (result.data ?? []) as HomeRecordTargetRow[]) {
    ...
    if (typeof row.recalculated_at === "string" && Number.isFinite(Date.parse(row.recalculated_at)) && (!recalculatedAt || Date.parse(row.recalculated_at) > Date.parse(recalculatedAt))) recalculatedAt = row.recalculated_at;
  }
  return { recalculatedAt, settingsMismatch, targets };
}
...
export function useHomeRecordTargets({
  ...
  useEffect(() => {
    ...
    async function loadForCurrentOwner() {
      ...
      setState({ ownerKey: `${loadUserId}:${timezone}:${logicalDayStart}`, snapshot: { error: null, loading: true, recalculatedAt: null, settingsMismatch: false, targets: {} } });
      try {
        ...
        if (generation !== generationRef.current) return;
        setState({ ownerKey: `${loadUserId}:${timezone}:${logicalDayStart}`, snapshot: { error: error instanceof Error ? error.message : "Record targets could not be loaded.", loading: false, recalculatedAt: null, settingsMismatch: false, targets: {} } });
      }
    }
