...
/** Deterministic JSON used for command identities and accepted payload digests. */
...
export function deterministicUuid(value: string): string {
  ...
  const versioned = `${hex.slice(0, 12)}4${hex.slice(13, 16)}${(parseInt(hex[16]!, 16) & 0x3 | 0x8).toString(16)}${hex.slice(17, 20)}${hex.slice(20, 32)}`;
