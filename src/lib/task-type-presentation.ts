...
export type TaskTypeAccentPresentation = {
  ...
  tableChildSurfaceClassName: string;
};
...
export function getTaskTypeTableChildSurfaceClassName(accentKey: unknown) {
  const accent = resolveTaskTypeAccent(accentKey);
  return `border-transparent ${accent.tableChildSurfaceClassName}`;
}
