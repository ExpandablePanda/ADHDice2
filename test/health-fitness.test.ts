...
test("all 7.11.40 release version surfaces stay aligned", () => {
  assert.equal(packageJson.version, "7.11.40");
  assert.equal(packageLock.version, "7.11.40");
  assert.equal(packageLock.packages[""].version, "7.11.40");
  assert.match(appVersionSource, /"version":\s*"7\.11\.40"/);
  assert.match(taskAppSource, /const APP_VERSION = "7\.11\.40"/);
