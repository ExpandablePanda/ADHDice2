...
test("all 7.12.96 release version surfaces stay aligned", () => {
  assert.equal(packageJson.version, "7.12.96");
  assert.equal(packageLock.version, "7.12.96");
  assert.equal(packageLock.packages[""].version, "7.12.96");
  assert.match(appVersionSource, /"version":\s*"7\.12\.96"/);
  assert.match(appVersionModuleSource, /APP_VERSION = "7\.12\.96"/);
  assert.match(currentStateSource, /Current working app version: `7\.12\.96`/);
