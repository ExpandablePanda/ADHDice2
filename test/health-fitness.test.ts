...
test("all 7.12.76 release version surfaces stay aligned", () => {
  assert.equal(packageJson.version, "7.12.76");
  assert.equal(packageLock.version, "7.12.76");
  assert.equal(packageLock.packages[""].version, "7.12.76");
  assert.match(appVersionSource, /"version":\s*"7\.12\.76"/);
  assert.match(taskAppSource, /const APP_VERSION = "7\.12\.76"/);
  assert.match(taskAppSource, /const HUD_VERSION = APP_VERSION/);
  assert.match(currentStateSource, /Current working app version: `7\.12\.76`/);
});
