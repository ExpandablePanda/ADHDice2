...
test("Attention decorates only the primary current-status presentation", () => {
  assert.match(statusUiSource, /options: \{ attention\?: boolean; className\?: string; glyphClassName\?: string; inverted\?: boolean \}/);
...
test("Attention preserves canonical status color and includes one decorative Bell overlay", () => {
  ...
  assert.match(statusUiSource, /options\.attention \? "task-status-circle-attention relative"/);
...
test("Attention animation has a slow repeating cycle and static reduced-motion fallbacks", () => {
  assert.match(globalStyleSource, /animation: task-status-attention-glow 5s ease-in-out infinite;/);
  assert.match(globalStyleSource, /animation: task-status-attention-status-glyph 5s ease-in-out infinite;/);
  assert.match(globalStyleSource, /animation: task-status-attention-bell-glyph 5s ease-in-out infinite;/);
  ...
  assert.match(globalStyleSource, /@media \(prefers-reduced-motion: reduce\)[\s\S]*\.task-status-circle-attention \{[\s\S]*animation: none;[\s\S]*box-shadow: 0 0 0 2px color-mix\(in srgb, currentColor/);
...
test("Low Stimulation mode resolves the Attention circle to one static status glyph", () => {
  assert.match(globalStyleSource, /\[data-lowstim\] \.task-status-circle-attention \{[\s\S]*animation: none !important;[\s\S]*box-shadow: 0 0 0 2px color-mix\(in srgb, currentColor/);
