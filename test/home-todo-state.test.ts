...
test("Home todo renders seven flat sortable sections, settings, and the recovered task behavior", () => {
  ...
  assert.equal((renderHomeTask.match(/className=\{HOME_TODO_ACTION_CLASS\}/g) ?? []).length, 1);
  assert.equal((renderHomeTask.match(/iconClassName=\{HOME_TODO_ACTION_ICON_CLASS\}/g) ?? []).length, 1);
...
test("Home row gear menus preserve actions, move-day destinations, and close behavior", () => {
  const source = readFileSync(new URL("../src/components/task-app/home-page.tsx", import.meta.url), "utf8");
  const renderStart = source.indexOf("function renderHomeTask");
  const renderEnd = source.indexOf("\n  useEffect", renderStart);
  const renderSource = source.slice(renderStart, renderEnd);
  const destinationStart = renderSource.indexOf("rowActionMenuView === \"move-day\"");
  ...
  assert.match(renderSource, /setRowActionMenu\(\(current\) => current\?\.taskId === task\.id \? null : \{ taskId: task\.id, view: "actions" \}\)/);
