...
test("Home todo renders the recovered ten-item, wrapped-title, status, and picker behavior", () => {
  ...
  assert.match(source, /grid min-w-0 grid-cols-\[auto_auto_auto_minmax\(0,1fr\)_auto_auto_auto\] items-center/);
  assert.match(source, /renderTaskStatusCircle\(displayStatus, "sm", \{ className: "!h-7 !w-7" \}\)/);
  assert.doesNotMatch(source, /flex shrink-0 flex-col items-center/);
  assert.doesNotMatch(source, /basis-full/);
  assert.match(source, /variant="rowToolbar"/);
