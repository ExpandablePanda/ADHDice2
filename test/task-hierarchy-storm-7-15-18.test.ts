...
test("server stale fencing is checked before the locking and ancestry work", () => {
  const precheck = hierarchySql.slice(
    hierarchySql.indexOf("select task.revision, task.canonical_revision"),
    hierarchySql.indexOf("select task.*", hierarchySql.indexOf("select task.revision, task.canonical_revision")),
  );
  assert.match(precheck, /using errcode = '40001'/);
  assert.doesNotMatch(precheck, /for update|while v_parent_cursor/);
  assert.match(hierarchySql, /--\s+locked read below repeats the fence/);
});
