...
test("one canonical full-screen loader gates auth restoration and initial app boot", async () => {
  ...
  assert.match(globalStyles, /@keyframes workspace-loading-ring-rotation[\s\S]*?transform: rotate\(0deg\);[\s\S]*?transform: rotate\(360deg\);/);
  assert.match(globalStyles, /\.workspace-loading-ring-rotator \{[\s\S]*?animation: workspace-loading-ring-rotation 2s linear infinite;/);
