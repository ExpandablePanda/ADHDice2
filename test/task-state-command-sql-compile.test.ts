...
test("7.11.77 Achievement deferral patch compiles over the installed schedule-replay RPC", (t) => {
  if (!host) {
    t.skip("set ADHDICE_SQL_COMPILE_PGHOST to run the disposable local PostgreSQL compile regression");
    return;
  }
  ...
  const scratch = mkdtempSync(join(tmpdir(), "adhdice-sql-compile-7-11-77-"));
  const database = `adhdice_compile_71177_${process.pid}_${Date.now()}`;
  const fixtureSetup = join(scratch, "fixture-setup.sql");
  const orderedSchema = join(scratch, "ordered-schema.sql");
  const baselineRpc = join(scratch, "baseline-command-rpc.sql");
  const verification = join(scratch, "verification.sql");
...
create function auth.uid() returns uuid language sql stable as $$ select null::uuid $$;
    ...
    const preDeferralRpc = run("git", ["show", "fbe6d79:supabase/add_task_state_command_rpc.sql"]);
    writeFileSync(baselineRpc, `${preDeferralRpc}\nrevoke all on function public.adhdice_execute_task_state_command(uuid, jsonb) from public, anon, authenticated;\ngrant execute on function public.adhdice_execute_task_state_command(uuid, jsonb) to service_role;\n`);
    run(psql, [...connectionArgs(database), "-v", "ON_ERROR_STOP=1", "-f", baselineRpc]);
    run(psql, [...connectionArgs(database), "-v", "ON_ERROR_STOP=1", "-f", join(repositoryRoot, "supabase/patch_task_state_schedule_auto_missed_7_11_73.sql")]);
    run(psql, [...connectionArgs(database), "-v", "ON_ERROR_STOP=1", "-f", join(repositoryRoot, "supabase/patch_task_state_achievement_deferral_7_11_77.sql")]);

    writeFileSync(verification, `with definition as (
  select pg_get_functiondef('public.adhdice_execute_task_state_command(uuid, jsonb)'::regprocedure) as body
)
select
  position($$set_config('adhdice.achievement_deferred_user_id', p_user_id::text, true)$$ in body) > 0,
  position($$for v_automatic_history in$$ in body) > position($$set_config('adhdice.achievement_deferred_user_id', p_user_id::text, true)$$ in body),
  position($$set_config('adhdice.achievement_deferred_user_id', '', true)$$ in body) > position($$for v_automatic_history in$$ in body),
  position($$set_config('adhdice.achievement_deferred_user_id', '', true)$$ in body) < position($$public.adhdice_evaluate_achievements($$ in body),
  (length(body) - length(replace(body, $$public.adhdice_evaluate_achievements($$, ''))) = length($$public.adhdice_evaluate_achievements($$),
  position($$Final Achievement evaluation failed$$ in body) > 0,
  has_function_privilege('service_role', 'public.adhdice_execute_task_state_command(uuid, jsonb)', 'execute'),
  not has_function_privilege('authenticated', 'public.adhdice_execute_task_state_command(uuid, jsonb)', 'execute'),
  not has_function_privilege('anon', 'public.adhdice_execute_task_state_command(uuid, jsonb)', 'execute')
from definition;
`);
    const result = run(psql, [...connectionArgs(database), "-At", "-v", "ON_ERROR_STOP=1", "-f", verification])
      .trim()
      .split("|");
    assert.deepEqual(result, ["t", "t", "t", "t", "t", "t", "t", "t", "t"]);
  } finally {
    try {
      run(dropdb, ["--if-exists", ...utilityConnectionArgs(), database]);
    } finally {
