...
test("inline child title input reuses visible Table title typography", () => {
  ...
  assert.match(primitivesSource, /TASK_TABLE_INLINE_TITLE_EDITOR_CLASS = `\$\{TASK_TABLE_VISIBLE_TITLE_TEXT_CLASS\} h-\[15px\] min-h-0 px-1 py-0`/);
  ...
  assert.match(inputSource, /TASK_TABLE_INLINE_TITLE_EDITOR_CLASS/);
  assert.match(inputSource, /style=\{TASK_TABLE_TITLE_RENAME_INPUT_TYPOGRAPHY_STYLE\}/);
  assert.doesNotMatch(inputSource, /h-7|h-\[28px\]|px-1\.5|text-\[13px\]|font-medium|leading-|tracking-/);
});
...
test("Table Step and Substep title-cell drafts retain the shared input and creation wiring", () => {
  ...
  assert.match(tableSource, /<TaskTitleDraftInput[\s\S]*className=\{`\$\{TASK_TABLE_INLINE_TITLE_EDITOR_CLASS\}/);
  assert.equal((tableSource.match(/style=\{TASK_TABLE_TITLE_RENAME_INPUT_TYPOGRAPHY_STYLE\}/g) ?? []).length, 4);
  assert.match(tableSource, /TASK_TABLE_INLINE_TITLE_EDITOR_CLASS/);
});
