...
do $drop_obsolete_records_reconciliation_overloads$
declare
  v_function record;
begin
  for v_function in
    ...
    where namespace.nspname = 'public' and procedure.proname = 'adhdice_reconcile_records'
  loop
    execute pg_catalog.format('drop function %I.%I(%s)', v_function.nspname, v_function.proname, v_function.identity_arguments);
  end loop;
end;
$drop_obsolete_records_reconciliation_overloads$;
...
begin
  ...
  select metric_key, scope_kind, nullif(scope_id, ''), nullif(title_snapshot, ''), value, unit,
    ...
    candidate_identity, first_achieved_at, evidence_fingerprint, evidence_snapshot
  from pg_catalog.jsonb_to_recordset(p_payload->'current_records') as item(
    ...
    first_achieved_at text, evidence_fingerprint text, evidence_snapshot jsonb
  );
  ...
  select metric_key, scope_kind, nullif(scope_id, ''), nullif(title_snapshot, ''), event_kind, value, unit,
    ...
    candidate_identity, evidence_fingerprint, evidence_snapshot, first_qualified_at, first_achieved_at
  from pg_catalog.jsonb_to_recordset(p_payload->'events') as item(
    ...
    candidate_identity text, evidence_fingerprint text, evidence_snapshot jsonb,
  ...
  on conflict (user_id, rules_version, metric_key, scope_kind, (coalesce(scope_id, ''))) do update set
    ...
    evidence_fingerprint = excluded.evidence_fingerprint, evidence_snapshot = excluded.evidence_snapshot,
