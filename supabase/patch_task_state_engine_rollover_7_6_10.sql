-- ADHDice 7.6.10 manual corrective install.
-- Apply after patch_task_state_engine_rollover_7_6_9.sql. This replaces the
-- per-plan-row RPC work with one staged, set-based transaction.

-- The result shape adds the committed deduplication count, so PostgreSQL
-- requires a drop/recreate rather than CREATE OR REPLACE. The install is a
-- single migration transaction; no dependent database object owns this RPC.
...
begin
  ...
  -- One lock per user serializes tabs/devices. The payload is parsed once into
  -- transaction-local staging tables; no task or History procedural loops run.
  ...
  -- Validate JSON text before every enum cast. Archive/Trash are deliberately
  -- not accepted at this rollover boundary.
  if exists (
  ...
  -- Lock eligible rows as one relation. Existing contradictory History wins;
  -- same-outcome rows are left to the unique constraint for idempotent replay.
