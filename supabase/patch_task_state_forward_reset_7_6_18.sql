-- ADHDice 7.6.18 forward-only recurring-date reset.
--
-- Run this file as one transaction. The temporary snapshot makes the preview the
-- exact input to the guarded update. Inspect the preview before allowing the
-- UPDATE to run; use ROLLBACK instead of COMMIT if any row is unexpected.
--
-- The only persistent write is adhdice_clean_tasks.due_on plus its optimistic
-- revision/update metadata. History, rewards, streaks, lifecycle, recurrence,
-- Archive, and Trash are never written.
...
-- 2. GUARDED UPDATE. Only rows classified eligible in the preview snapshot can change.
with updated as (
  update public.adhdice_clean_tasks as task
  set
    due_on = snapshot.proposed_forward_due_on,
    revision = snapshot.revision + 1,
    updated_at = statement_timestamp()
  from adhdice_task_state_forward_reset_7_6_18_snapshot as snapshot
  where snapshot.eligible
    and snapshot.task_exists
    and snapshot.affected_id_is_unique
    and snapshot.active_recurring_lifecycle
    and snapshot.current_date_is_affected_corruption
    and snapshot.recurrence_configuration_valid
    and snapshot.proposed_date_valid_under_recurrence
    and snapshot.proposed_forward_due_on >= snapshot.logical_date
    and snapshot.proposed_forward_due_on < snapshot.current_due_on
    and task.id = snapshot.task_id
    and task.user_id = snapshot.user_id
    and task.revision = snapshot.revision
    and task.updated_at = snapshot.updated_at
    and task.status::text = snapshot.current_status
    and task.status::text not in ('complete', 'archived', 'trashed')
    and task.trashed_at is null
    and task.due_on = snapshot.current_due_on
    and task.due_on = snapshot.expected_corrupted_due_on
    and task.repeat_frequency::text = snapshot.recurrence_type
    and task.repeat_interval = snapshot.recurrence_interval
    and task.repeat_days_of_week is not distinct from snapshot.recurrence_weekdays
    and task.repeat_day_of_month is not distinct from snapshot.repeat_day_of_month
    and task.repeat_monthly_mode::text is not distinct from snapshot.repeat_monthly_mode
    and task.repeat_monthly_ordinal::text is not distinct from snapshot.repeat_monthly_ordinal
    and task.repeat_monthly_weekday is not distinct from snapshot.repeat_monthly_weekday
    and task.due_on is distinct from snapshot.proposed_forward_due_on
  returning
    task.id as task_id,
    snapshot.current_due_on as previous_due_on,
    task.due_on as reset_due_on,
    snapshot.revision as previous_revision,
    task.revision as reset_revision
)
select
  updated.*,
  count(*) over () as updated_count
from updated
order by task_id;

-- 3. POST-UPDATE VERIFICATION. No non-Task surfaces are queried or written.
select
  snapshot.task_id,
  snapshot.title,
  snapshot.current_due_on as preview_due_on,
  snapshot.proposed_forward_due_on,
  task.due_on as verified_due_on,
  snapshot.revision as preview_revision,
  task.revision as verified_revision,
  snapshot.eligible as preview_eligible,
  case
    when not snapshot.task_exists then 'missing'
    when snapshot.repair_classification = 'unchanged'
      and task.due_on = snapshot.proposed_forward_due_on then 'unchanged'
    when snapshot.eligible
      and task.due_on = snapshot.proposed_forward_due_on
      and task.revision = snapshot.revision + 1 then 'updated'
    when snapshot.eligible then 'guarded update skipped or verification failed'
    else 'skipped'
  end as verification_result,
  count(*) over () as expected,
  count(*) filter (where task.id is not null) over () as found,
  count(*) filter (
    where snapshot.eligible
      and task.due_on = snapshot.proposed_forward_due_on
  ...
  count(*) filter (
    where snapshot.repair_classification = 'unchanged'
      and task.due_on = snapshot.proposed_forward_due_on
  ) over () as unchanged,
  count(*) filter (where snapshot.repair_classification = 'skipped') over () as skipped,
  count(*) filter (where task.id is null) over () as missing
from adhdice_task_state_forward_reset_7_6_18_snapshot as snapshot
left join public.adhdice_clean_tasks as task
