-- ADHDice 7.6.13 manual corrective install.
-- Apply after patch_task_state_engine_rollover_7_6_12.sql. This retains its
-- set-based and deferred-achievement behavior while rejecting effective task
-- no-ops at the final SQL write boundary.
...
begin
  ...
  create temporary table pg_temp.adhdice_rollover_eligible_tasks (
    ...
    expected_revision integer not null,
    patch jsonb not null,
    status_target public.adhdice_clean_task_status,
    due_on_target date,
    completed_at_target timestamptz,
    active_status_logical_date_target date,
    active_occurrence_due_on_target date
  ) on commit drop;
  ...
  if exists (
    ...
    where patch ? 'status'
      and coalesce(btrim(patch->>'status'), '') not in ('unscheduled', 'pending', 'in_progress', 'done', 'missed', 'did_my_best', 'upcoming', 'not_due', 'delayed', 'complete')
  ) then
  ...
  insert into pg_temp.adhdice_rollover_eligible_tasks (
    task_id, expected_revision, patch, status_target, due_on_target,
    completed_at_target, active_status_logical_date_target,
    active_occurrence_due_on_target
  )
  select task.id, plan.expected_revision, plan.patch,
    case when plan.patch ? 'status' then case when btrim(plan.patch->>'status') = 'unscheduled'
      then 'pending'::public.adhdice_clean_task_status
      else btrim(plan.patch->>'status')::public.adhdice_clean_task_status end else null end,
    case when plan.patch ? 'dueOn' then nullif(btrim(plan.patch->>'dueOn'), '')::date else null end,
    case when plan.patch ? 'completedAt' then nullif(btrim(plan.patch->>'completedAt'), '')::timestamptz else null end,
    case when plan.patch ? 'activeStatusLogicalDate' then nullif(btrim(plan.patch->>'activeStatusLogicalDate'), '')::date else null end,
    case when plan.patch ? 'activeOccurrenceDueOn' then nullif(btrim(plan.patch->>'activeOccurrenceDueOn'), '')::date else null end
  ...
  set
    status = case when eligible.patch ? 'status' then eligible.status_target else task.status end,
    due_on = case when eligible.patch ? 'dueOn' then eligible.due_on_target else task.due_on end,
    completed_at = case when eligible.patch ? 'completedAt' then eligible.completed_at_target else task.completed_at end,
    active_status_logical_date = case when eligible.patch ? 'activeStatusLogicalDate' then eligible.active_status_logical_date_target else task.active_status_logical_date end,
    active_occurrence_due_on = case when eligible.patch ? 'activeOccurrenceDueOn' then eligible.active_occurrence_due_on_target else task.active_occurrence_due_on end,
  ...
  where task.id = eligible.task_id
    ...
    and (
      (eligible.patch ? 'status' and task.status is distinct from eligible.status_target)
      or (eligible.patch ? 'dueOn' and task.due_on is distinct from eligible.due_on_target)
      or (eligible.patch ? 'completedAt' and task.completed_at is distinct from eligible.completed_at_target)
      or (eligible.patch ? 'activeStatusLogicalDate' and task.active_status_logical_date is distinct from eligible.active_status_logical_date_target)
      or (eligible.patch ? 'activeOccurrenceDueOn' and task.active_occurrence_due_on is distinct from eligible.active_occurrence_due_on_target)
    );
