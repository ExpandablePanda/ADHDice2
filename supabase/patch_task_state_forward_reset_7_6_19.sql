-- ADHDice 7.6.20 correction for the exact 26 cursors re-advanced after 7.6.18.
-- Three exact weekly Monday rows advanced twice; the other 23 advanced once.
-- Preview the transaction-local snapshot before allowing the guarded UPDATE.
-- The only persistent fields written are due_on, revision, and updated_at.
...
-- 1. PREVIEW ONLY. Expect 26 eligible before correction, then 26 unchanged after it.
select
...
-- 2. SEPARATE GUARDED UPDATE.
with updated as (
  update public.adhdice_clean_tasks as task
  set
    due_on = snapshot.corrected_due_on,
    revision = snapshot.expected_revision + 1,
    updated_at = statement_timestamp()
  from adhdice_task_state_forward_reset_7_6_19_snapshot as snapshot
  where snapshot.eligible
    and snapshot.task_exists
    and snapshot.expected_id_is_unique
    and snapshot.active_recurring_lifecycle
    and snapshot.recurrence_configuration_unchanged
    and snapshot.corrected_forward_boundary_valid
    and snapshot.exact_expected_recurrence_transitions
    and task.id = snapshot.task_id
    and task.user_id = snapshot.user_id
    and task.due_on = snapshot.expected_readvanced_due_on
    and task.revision = snapshot.expected_revision
    ...
    and task.due_on is distinct from snapshot.corrected_due_on
  returning task.id, snapshot.expected_readvanced_due_on as previous_due_on,
    task.due_on as corrected_due_on, snapshot.expected_revision as previous_revision,
    task.revision as corrected_revision
)
select updated.*, count(*) over () as updated_count
from updated
order by id;
...
select
  snapshot.task_id,
  snapshot.expected_readvanced_due_on,
  snapshot.corrected_due_on,
  ...
  case
    ...
    when task.due_on = snapshot.corrected_due_on and task.revision = snapshot.expected_revision + 1 then 'corrected'
    else 'guarded update skipped or verification failed'
  end as verification_result,
  count(*) over () as expected,
  count(*) filter (where task.due_on = snapshot.corrected_due_on and task.revision = snapshot.expected_revision + 1) over () as corrected,
  count(*) filter (where task.id is null) over () as missing
from adhdice_task_state_forward_reset_7_6_19_snapshot as snapshot
left join public.adhdice_clean_tasks as task
  on task.id = snapshot.task_id and task.user_id = snapshot.user_id
