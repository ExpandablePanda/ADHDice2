-- ADHDice 7.6.12 manual corrective install.
-- Apply after patch_task_state_engine_rollover_7_6_11.sql. This retains its
...
begin
  ...
  where task.id = eligible.task_id
    ...
    and (
      (eligible.patch ? 'status' and task.status is distinct from (
        case when eligible.patch->>'status' = 'unscheduled'
          then 'pending'::public.adhdice_clean_task_status
          else (eligible.patch->>'status')::public.adhdice_clean_task_status
        end
      ))
      or (eligible.patch ? 'dueOn' and task.due_on is distinct from nullif(eligible.patch->>'dueOn', '')::date)
      or (eligible.patch ? 'completedAt' and task.completed_at is distinct from nullif(eligible.patch->>'completedAt', '')::timestamptz)
      or (eligible.patch ? 'activeStatusLogicalDate' and task.active_status_logical_date is distinct from nullif(eligible.patch->>'activeStatusLogicalDate', '')::date)
      or (eligible.patch ? 'activeOccurrenceDueOn' and task.active_occurrence_due_on is distinct from nullif(eligible.patch->>'activeOccurrenceDueOn', '')::date)
    );
