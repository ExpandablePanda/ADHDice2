-- ADHDice 7.15.33 Current Task Projection V3 semantic convergence contract.
--
-- The schema remains V2. Only the durable calculator algorithm changes.
-- Existing V1/V2 rows remain readable but are stale until the trusted
-- calculator rebuilds them; this migration never writes projection rows.
...
begin
  if to_regclass('public.adhdice_task_current_projections') is null then
    raise exception 'Current Task projection V3 requires the existing projection table.';
  end if;
end;
$guard$;
...
alter table public.adhdice_task_current_projections
  ...
  add constraint adhdice_task_current_projection_algorithm_version_allowed
    check (projection_algorithm_version in ('task-current-projection-algorithm-v1', 'task-current-projection-algorithm-v2', 'task-current-projection-algorithm-v3')),
...
begin
  ...
  if v_candidate.canonical_task_revision is distinct from v_task.canonical_revision then
    raise exception 'Current Task projection canonical Task revision is stale.' using errcode = '40001';
  end if;
  ...
  if not found or v_candidate.history_sync_epoch is distinct from v_sync_epoch then
    raise exception 'Current Task projection History sync epoch is stale.' using errcode = '40001';
  end if;
  ...
  if v_candidate.history_source_revision is distinct from v_latest_history_revision then
    raise exception 'Current Task projection entity History frontier is stale.' using errcode = '40001';
  end if;
  ...
  if v_candidate.logical_day_settings_revision is distinct from v_profile_settings_revision then
    raise exception 'Current Task projection logical-day settings revision is stale.' using errcode = '40001';
  end if;
  if v_candidate.projected_logical_date is distinct from public.adhdice_effective_logical_date(
    ...
    raise exception 'Current Task projection logical date is stale.' using errcode = '40001';
  end if;
  ...
  if not found
    ...
    raise exception 'Current Task projection schedule or behavior source fence is stale.' using errcode = '40001';
  end if;
  ...
  if not found then
    raise exception 'Current Task projection candidate is older than the stored projection or would downgrade the current algorithm.' using errcode = '40001';
  end if;
  return v_written;
end;
$function$;
...
begin
  if current_user <> 'service_role' then
    raise exception 'Current Task projection V3 candidates are backend-only.' using errcode = '42501';
  end if;
  if p_user_id is null then raise exception 'A user is required.' using errcode = '22023'; end if;
  if p_limit is null or p_limit < 1 or p_limit > 10 then
    raise exception 'Current Task projection V3 candidate limit must be between 1 and 10.' using errcode = '22023';
  end if;
  return query
  select task.id
    from public.adhdice_clean_tasks task
   where task.user_id = p_user_id
     ...
     and (
       not exists (select 1 from public.adhdice_task_current_projections projection
        where projection.user_id = task.user_id and projection.entity_id = task.id)
       or exists (select 1 from public.adhdice_task_current_projections projection
        where projection.user_id = task.user_id and projection.entity_id = task.id
          and (projection.validity <> 'valid'
            or projection.projection_schema_version <> 'task-current-projection-schema-v2'
...
begin
  if current_user <> 'service_role' then
    raise exception 'Current Task projection V3 count is backend-only.' using errcode = '42501';
  end if;
  if p_user_id is null then raise exception 'A user is required.' using errcode = '22023'; end if;
  return (
    select count(*)
