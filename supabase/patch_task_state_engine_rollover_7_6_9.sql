-- ADHDice 7.6.9 manual corrective install.
-- Apply after patch_task_state_engine_rollover_7_6_7.sql. This retains the
-- existing RPC contract without adding the engine-only `unscheduled` value to
-- adhdice_clean_task_status.
  ...
  loop
    ...
    if v_patch ? 'status' then
      ...
      if v_status = 'unscheduled' then
        v_patch := jsonb_set(v_patch, '{status}', '"pending"'::jsonb);
      elsif v_status not in ('pending', 'in_progress', 'done', 'missed', 'did_my_best', 'upcoming', 'not_due', 'delayed', 'archived', 'trashed', 'complete') then
        raise exception 'Unsupported task status in engine rollover plan: %', v_status;
      end if;
    end if;

    -- Validate text before any enum cast so engine-only/unknown History cannot
    -- reach adhdice_clean_task_status through a malformed client payload.
    for v_history in select value from jsonb_array_elements(coalesce(v_item->'history', '[]'::jsonb))
    loop
      v_outcome := v_history->>'outcome';
      if v_outcome not in ('done', 'did_my_best', 'missed', 'delayed', 'complete') then
        raise exception 'Unsupported History outcome in engine rollover plan: %', v_outcome;
      end if;
    end loop;
