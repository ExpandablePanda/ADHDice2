...
| Table inline action rows | `src/components/ui/task-table-primitives.tsx` | Shared Task Table action-row geometry, viewport compensation, and compact shell | Domain content and mutations remain local | Active shared presentation source |
| List quick-panel shell | `src/components/ui/task-table-primitives.tsx` | Shared Task List quick-edit shell | List behavior and domain mutations remain local | Active shared presentation source |
