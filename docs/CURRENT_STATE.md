...
## Current App Version
- Current working app version: `6.13.19`.
...
## 6.13 Checkpoint
- `6.13.19` is a narrow Table View scroll-jump correction: the actual Table scroll owner now disables native browser overflow anchoring, so status and reorder-prone metadata row reflow can no longer auto-shift the Table viewport before the existing Table-local restore logic runs. The edited row can still reorder in the background, but normal Table View no longer follows the browser's implicit section-anchor jump. List behavior and overlay-only reveal behavior stay unchanged.
