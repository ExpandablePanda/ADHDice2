...
| Older economy helper | [`useEconomy.ts`](../../src/hooks/useEconomy.ts) | The obsolete direct Task reward-roll/claim helper has been removed. Canonical Task rewards flow through entitlement fulfillment; `useEconomy` retains profile/economy event helpers only. |
...
- The obsolete direct roll/claim/profile/ledger path has been retired. It is not a supported compatibility route; canonical Task rewards use entitlement fulfillment.
...
- `adhdice_task_reward_claims` has a unique index for `(user_id, task_id, reward_date)`, covering canonical parent, Step, and Substep entity IDs.
...
- The retired direct reward path is not part of the current contract. Canonical reward entitlement fulfillment creates durable grant/pending-dice evidence before the current claim RPC applies economy effects.
